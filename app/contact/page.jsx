          ...
          <div className="bg-surface border border-border rounded-2xl shadow-2xl shadow-black/20 p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input type="text" id="name" name="name" placeholder="e.g., Ada Lovelace" className="w-full px-4 py-3 rounded-lg bg-background/70 border border-border placeholder:text-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary" required />
              </div>
              <div>
                <label htmlFor="email" className="block text-sm font-medium text-foreground/90 mb-1">Email</label>
                <input type="email" id="email" name="email" placeholder="e.g., ada.lovelace@example.com" className="w-full px-4 py-3 rounded-lg bg-background/70 border border-border placeholder:text-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary" required />
              </div>
              <div>
                <label htmlFor="message" className="block text-sm font-medium text-foreground/90 mb-1">Message</label>
                <textarea id="message" name="message" rows="4" placeholder="Your message here..." className="w-full px-4 py-3 rounded-lg bg-background/70 border border-border placeholder:text-foreground/50 resize-none focus:outline-none focus:ring-2 focus:ring-primary" required />
              </div>
              <MotionDiv whileTap={{ scale: 0.98 }} className="pt-2">
                <button type="submit" className="w-full flex items-center justify-center gap-3 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold py-3 px-6 rounded-lg transition-colors disabled:opacity-50" disabled={loading}>
