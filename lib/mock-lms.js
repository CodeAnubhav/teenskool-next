...
export const XP_RULES = {
    ...
    ONBOARDING_COMPLETE: 500, // Instant boost to almost Founder level
};

// 3. ONBOARDING QUIZ (5 Mandatory Questions)
export const ONBOARDING_QUIZ = [
    {
        ...
        correct_index: 1, // Identifying a Problem
        points: 100
    },
    {
        id: "q2",
    ...
    },
    {
        ...
        correct_index: 1, // Minimum Viable Product
        points: 100
    },
    {
        id: "q4",
...
// 4. MOCK USER PROFILE (Simulating DB Fetch)
// In a real app, this would fetch from Supabase 'profiles' table.
export const getMockUserProfile = (user) => {
    if (!user) return null;

    // Checking local storage to simulate persistent state between refreshes for the demo
    let storedProfile = null;
    if (typeof window !== "undefined") {
        const stored = localStorage.getItem(`mock_profile_${user.email}`);
        if (stored) {
    ...
    return storedProfile || {
        ...
        system_role: "student",
        game_role_id: "role_novice", // Default
        total_xp: 0,
...
// 5. MOCK UPDATE FUNCTION
export const updateMockUserProfile = (user, updates) => {
    if (!user) return;
    const current = getMockUserProfile(user);
    const updated = { ...current, ...updates };

    // Simulate DB update
    if (typeof window !== "undefined") {
        localStorage.setItem(`mock_profile_${user.email}`, JSON.stringify(updated));
    }
    return updated;
};

// Helper to get Game Role details by ID
export const getGameRoleById = (id) => GAME_ROLES.find(r => r.id === id) || GAME_ROLES[0];

// Helper to calculate next level progress
export const getLevelProgress = (xp) => {
    ...
    if (!nextRole) return { progress: 100, next_xp: xp }; // Max level
